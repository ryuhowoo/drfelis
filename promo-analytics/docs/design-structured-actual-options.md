...
## 5. 공헌이익 (5번 문제) — 옵션 단위 실측 분해 + 캠페인 ground truth 병기

```
옵션 공헌이익 = 매출 − 수수료(실측) − 원가(실측) − 물류비 − 광고비
  · 수수료·원가: 실적옵션 멤버 합산(실측)
  · 물류비:   매출 × 물류율(평균 12%)         ← 결정①: 현행 % 유지(per-box 폐기)
  · 광고비:   캠페인 실제 광고비 × (옵션매출 / 전체매출)   ← 결정②: ad_spend 실측 배분, 없으면 레이트카드 광고율 폴백
```
- 캠페인 단위 **ground truth 병기**: `contribution_amount`(기간 내 공식몰 전체 공헌이익, 정기구독 포함)를 헤드라인에 함께 표시 → 옵션 분해 합과 대조(reconciliation).
- 분해를 UI에 노출 → "어디서 마진이 깎였나" 추적.
- **신규 입력**: `promotions.ad_spend`(실제 광고비). 전체 공헌이익은 기존 `contribution_amount` 재사용.
...
### Phase 3 — 수집 정체성 + 도구 격하  ✅ 구현됨
**범위**
- 업로드 정체성: ②실적·⑤가이드를 **파일명 캠페인 코드(CF_P_…)로 자동 결속** — 정상 경로를 업로드 카드 카피로 명시(별도 명시 선택 UI 없이 코드가 식별자). 코드 일치 시 한 캠페인에 적층.
- `actual_promotion_id` 비교연결 → 결속 충족 시 **접힘(details)** + '보정·관리자용' 라벨로 격하, 미충족일 때만 펼침. 병합 도구 → '관리자/정리용 · 레거시 보정' 라벨로 격하(함수는 유지).

**구현**: `upload/page.tsx`(카피), `promotions/[id]/page.tsx`(연결 섹션 격하), `edit/MergeForm.tsx`(병합 라벨).
**후속(선택)**: 코드가 근본적으로 어긋나는 케이스용 업로드-시점 드롭다운 선택 — 현 코드 결속으로 대부분 해소되어 보류.
...
## 9. 열린 결정 — 확정됨 (2026-06-17)

1. **물류비 = 매출의 12% 유지.** 물류사 제공 비용이 평균적으로 판매액의 12%. → per-box 단가 도입 폐기, 기존 레이트카드 `mult`(% 방식) 그대로. 5장 공헌이익 공식의 물류비 항은 매출×물류율로 유지(현행 유지, 변경 없음).
2. **캠페인 실제 광고비 = 캠페인 상세에서 직접 입력** (신규 `promotions.ad_spend`). 추가로 **실제 공헌이익액**(해당 기간 공식몰 전체 = 정기구독 포함, 유일하게 정확 계산 가능한 값)도 직접 입력 → 이미 존재하는 `promotions.contribution_amount` 재사용(라벨만 '기간 내 공식몰 전체 공헌이익액'으로 명확화). 이 값은 옵션 단위 분해의 **정합 기준(ground truth)** 으로 헤드라인에 병기.
3. **`옵션코드`는 ②·⑤ 완전 동일 체계 아님** → option_code 직접 조인은 채택하지 않고 **`match_signature`(정규화 SKU명:수량) 매칭을 1차로 유지**. 단, 가이드(플랜) 작성이 더 유동적이므로 **향후 옵션명 정합 규약**(가이드 옵션명 = 실적 노출 옵션명)을 점진 도입 → 정합 시 `match_source='label'` 신뢰도 상승. 체계는 운영하며 구축.

> 결정 반영 요약: 5장 물류비는 현행 % 유지(설계 단순화), 공헌이익 분해의 신규 입력은 **광고비(실측)** 와 **전체 공헌이익(ground truth)** 두 개의 캠페인 단위 값. option_code는 보조 신호로만.
