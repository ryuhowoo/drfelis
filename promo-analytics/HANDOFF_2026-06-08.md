...
> 🔴 **(2026-06-09 N5 개정)** 가이드 임포터는 **promotions를 생성/매칭하지 않는다** — `campaign_plans`에 plan row만 적재(코드 기준 lookup, 0021 독립 스키마). '플랜 만들기' 버튼의 빈 draft 자동 생성도 제거. 플랜↔실적 비교는 `actual_promotion_id` 명시적 짝짓기로만. 상세: 루트 `N5_단독시작문서.md`.
